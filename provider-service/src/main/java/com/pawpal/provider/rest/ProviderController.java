...
public class ProviderController {
    ...
    /**
     * GET /services/by-user/{userId}
     * Get a provider's service profile by their auth userId.
     * Access: Public
     */
    @GetMapping("/services/by-user/{userId}")
    public ResponseEntity<Object> getServiceByUserId(@PathVariable int userId) {
        Optional<Provider> found = providerService.getProviderByUserId(userId);
        if (found.isEmpty())
            return ResponseEntity.status(404).body("No service profile found for this user");
     ...
     */
    @PutMapping("/internal/services/{sid}/rating")
